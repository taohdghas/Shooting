...
PixelShaderOutput main(VertexShaderOutput input)
{
    ...
    if (gMaterial.enableLighting != 0)//Lightingする場合
    {
      ...
      //float3 pointreflectLight = reflect(pointLightDirection, normalize(input.normal));
       ...
       //float spotcos = pow(spotNdotH * 0.5f + 0.5f, 2.0f);
     //float spotreflectLight = reflect(spotLightDirectionOnSurface, normalize(input.normal));
     //float spotRdotE = dot(spotreflectLight, toEye);
        ...
        diffusePointLight + specularPointLight + diffuseSpotLight + specularSpotLight;
